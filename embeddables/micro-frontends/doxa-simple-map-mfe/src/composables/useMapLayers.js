...
// 
// QA Reference: Round 1 Q4 - Composable decomposition (lines 227-231)
// Design Flaw Fix: Extracts layer logic from MapComponent (~400 lines)
//
 ...
 * - addLanguageFamilyLayer() (lines 1332-1592)
 * - addRegionsLayer() (lines 1593-1712)
 * - addPeopleGroupsLayer() (lines 1714-1800+)
 * 
...
export function useMapLayers(options = {}) {
    ...
    function addLanguageFamilyLayer(peopleGroups, colorMode = 'languageFamily') {
        ...
        // peoplePraying is stored in GeoJSON feature properties (line ~97) as a number,
        // so the Mapbox ['get', 'peoplePraying'] expression reads directly from properties.
        // NOTE: feature-state path was removed — setFeatureState() was never called,
        // so ['feature-state', 'peoplePraying'] always returned null → all pins red.
        // If polling is implemented in the future, update the GeoJSON source data instead.
        ...
        const features = peopleGroups.map(pg => ({
            ...
            properties: {
                ...
                doxaRegion: pg._normalized?.doxaRegion || pg.doxaRegion || 'Unknown',
                // Country fields — `countryName` is the unified decoded display label
                // (DataSourceManager populates this via pray-tools .label OR the
                // local ISO3→name lookup table). Raw code + label kept for filters.
                country:         pg.country,
                ...
                // Parse to number so the Mapbox '>' expression works correctly.
                peoplePraying: Number(pg.peoplePraying ?? pg._normalized?.peoplePraying ?? 0),
                // engagementStatus / adoptionStatus: stored as booleans so Mapbox
                // ['==', ['get', 'engagementStatus'], true] expressions work correctly.
     ...
     * Shows DOXA regions as colored fill polygons
     * 
    ...
    function addRegionsLayer(regionsData, colorScheme = 'doxa-regions') {
        ...
        map.addLayer({
            ...
            paint: {
                ...
                'fill-opacity': colorScheme === 'none' ? 0.2 : 0.5
            }
        });
     ...
     * @param {string} sourceId - Optional source ID (defaults to layerId)
     */
