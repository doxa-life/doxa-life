...
> One of the three customization seams (`colors/`, `api/`, **`i18n/`**).
...
i18n/
  ...
  locales/
    en/                 ← one folder per locale (the fallback; fullest set — 10 areas)
      buttons.json  legend.json  detail.json  options.json
      search.json   messages.json  aria.json  prayerLaps.json
      tabs.json     popup.json
    es/  ar/  zh/  …     ← same key structure; some locales carry fewer area
                           files (see the locale contract below)
```
...
- **8 core area files in every locale**, identical names across locales: `buttons`, `legend`,
  `detail`, `options`, `search`, `messages`, `aria`, `prayerLaps`. Two more areas are partially
  translated: `tabs` (in `ar`/`de`/`en`/`es`/`fr`/`pt`/`ru`; `hi`/`it`/`ro`/`zh` fall back to
  English) and `popup` (in `en`/`fr` only). So the on-disk matrix is: `en`/`fr` 10 files,
  `ar`/`de`/`es`/`pt`/`ru` 9, `hi`/`it`/`ro`/`zh` 8. The filename (minus `.json`) becomes the
...
- **`en/` is the source of truth and the fallback.** Mirror its key structure exactly; keys may
  nest (`buttons.speedOption.slow`). Missing keys — and whole missing area files — fall back to
  English silently (`fallbackWarn`/`missingWarn` are off in the IIFE bundle).
...
1. **Copy `locales/en/` to `locales/<code>/`** (ISO-639-1, e.g. `sw`). Keep all 10 area files
   (`en/` carries the fullest set, including `tabs` + `popup`).
