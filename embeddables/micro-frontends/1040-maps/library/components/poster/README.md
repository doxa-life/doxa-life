...
- The preview is fed by `useMapPoster()` (in `library/composables/poster/useMapPoster.js`).
- Page sizes come from `library/constants/posterSizes.js`.
- Defaults (font, padding, theme) come from `library/constants/posterDefaults.js`.

## Cross-references

- Composables → `../../composables/poster/useMapPoster.js`, `usePosterLayout.js`, `useStaticImage.js`
...
If you add a new poster size, edit `constants/posterSizes.js`. If you add a new slot type, drop a `.vue` into `./slots/` and document in that folder's README.