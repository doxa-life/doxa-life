...
`index.js` entry, its `profiles/*.vue` screen(s), and optionally its own
`data/*.json`.

> **Terminology note:** the word "profile" is used at two levels.
> 1. A **bundle / app-profile** = a folder under `app-profiles/`.
> 2. A **profile screen** = one `.vue` file under that folder's `profiles/`.
...
│   │   └── profiles/
│   │       └── doxa-simple-map.vue   ← the profile screen
...
//     the profiles/ folder; ProfileLoader (in @map) cannot reach across.
...
- `import.meta.glob('./profiles/*.vue')` is resolved by Vite **at build time,
  relative to this file**, so it captures *only this bundle's* screens. That glob
...
### Step 4 — write your profile screen `profiles/my-new-map.vue`
...
<doxa-map profile-config='{
  "profile":   "doxa-simple-map",        // which screen in profiles/ to render (required)
...
- `profile` **must** match a file in this bundle's `profiles/` folder. Mismatch →
...
this and was superseded — don't reintroduce it without re-deciding; see
...
| Add a screen to an existing bundle | drop a `.vue` in that bundle's `profiles/`, request it via `profile-config.profile` |
