...
// Shared light/dark theme (provided by ProfileLoader). Boot the basemap on the
// persisted theme so this profile matches the UI + the other maps on reload.
...
watch(isMapReady, (ready) => {
  if (!ready) return
  // Apply the persisted theme's lightPreset now the Standard style has loaded (this map
  // has no toggle — boot-only). (bug-ghost-rings-standard-style — Option B.)
