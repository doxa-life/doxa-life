...
useShadowStyles(`
  ...
  /* CLONE EDIT: admin-2 floating control (top-left, below geocoder) */
  .admin2-control { position:absolute;top:60px;left:8px;width:300px;background:rgba(17,24,39,0.94);border:1px solid #1f2937;border-radius:8px;padding:10px 12px;color:#e5e7eb;font:12px/1.4 -apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;z-index:1300;box-shadow:0 4px 18px rgba(0,0,0,0.5); }
  .admin2-control label { display:flex;align-items:center;gap:8px;cursor:pointer; }
  .admin2-control input[type=checkbox] { accent-color:#60a5fa;cursor:pointer; }
  .admin2-status { margin-top:6px;font-size:10.5px;color:#9ca3af; }
  .admin2-status.err { color:#f87171; }
  .admin2-status b { color:#fbbf24;font-weight:600; }
  .admin2-actions { margin-top:8px;display:flex;gap:6px;flex-wrap:wrap; }
  .admin2-actions button { background:#1f2937;border:1px solid #374151;color:#e5e7eb;padding:5px 10px;font-size:11px;border-radius:4px;cursor:pointer; }
  .admin2-actions button:disabled { color:#6b7280;cursor:not-allowed; }
  .admin2-actions button:hover:not(:disabled) { background:#374151; }
...
// CLONE EDIT (my-upg-100-list bundle): narrowed to JUST the Regions tab.
// The original research-map.vue (in doxa-research-map bundle) keeps all 7 tabs;
// this clone is owned by my-upg-100-list and exposes only doxa-regions because
// our parent profile (upg-100-list.vue) wraps this as its own "Regions" tab.
const ALL_TABS = [
...
// Hide the inner tab bar — the parent profile (upg-100-list) owns the visible tab bar.
...
// loc-012: pass the active locale so list/search API data matches the selected
// language (same `lang` param as the people-group detail fetch). null → API default.
const _apiLocale = (() => { const v = inject('lang', null)?.value; return (v && v !== 'en') ? v : null })()
const dsm = new DataSourceManager({ sourcesConfig, locale: _apiLocale })
...
// Country boundary OUTLINE on country search — reuses the country-boundaries-v1
// tileset (same polygon the Regions tab paints from), shown via useCountryOutline.
// Mirrors doxa-simple-map's wiring so a country geocoder pick outlines the country.
...
async function openPacket() {
  ...
  try {
    ...
    if (groups.length > 50) {
      console.warn(`[research-map-clone] packet export truncated ${groups.length} → 50 people groups`)
    ...
    console.error('[research-map-clone] packet export failed:', err)
  } finally {
    _packetBusy = false
  }
}
...
function onGeocoderAggregateResult(evt) {
  ...
  // Country pick → outline the country boundary (country-boundaries-v1, the same
  // polygon the Regions tab uses). Any non-country pick clears a stale outline.
  if (evt.kind === 'country') {
...
function onGeocoderClear() {
  ...
  clustering.setSelectionFilter(null)
  // Drop any country boundary outline left by a country search pick.
  countryOutline.clearCountry()
...
// ─── CLONE EDIT (my-upg-100-list bundle): admin-2 layer for Asia unlock ──────
// Loads geoBoundaries IND + CHN admin-2 polygons on demand when the user
// flips the toggle. Click any admin-2 polygon on the map to add it to a draft
// list — supports composing India/China posters geographically.
const admin2Enabled = ref(false)
const admin2Loading = ref(false)
const admin2Error   = ref('')
const admin2Loaded  = ref({ IND: false, CHN: false })
const admin2Selected = ref(new Set())

const ADMIN2_SOURCES = {
  IND: 'https://media.githubusercontent.com/media/wmgeolab/geoBoundaries/9469f09592ced973a3448cf66b6100b741b64c0d/releaseData/gbOpen/IND/ADM2/geoBoundaries-IND-ADM2_simplified.geojson',
  CHN: 'https://media.githubusercontent.com/media/wmgeolab/geoBoundaries/9469f09592ced973a3448cf66b6100b741b64c0d/releaseData/gbOpen/CHN/ADM2/geoBoundaries-CHN-ADM2_simplified.geojson',
}
const ADMIN2_CACHE = {}

async function loadAdmin2(iso3) {
  if (ADMIN2_CACHE[iso3]) return ADMIN2_CACHE[iso3]
  const r = await fetch(ADMIN2_SOURCES[iso3])
  if (!r.ok) throw new Error(`Fetch ${iso3} admin-2 failed: HTTP ${r.status}`)
  const fc = await r.json()
  for (const f of fc.features || []) {
    f.properties = f.properties || {}
    f.properties._iso3 = iso3
    f.properties._admin2_id = `${iso3}-${f.properties.shapeID || f.properties.shapeName || Math.random().toString(36).slice(2, 8)}`
  }
  ADMIN2_CACHE[iso3] = fc
  return fc
}

function admin2ClickHandlerFactory(iso3) {
  return (e) => {
    if (!e.features || !e.features.length) return
    const id = e.features[0].properties._admin2_id
    const next = new Set(admin2Selected.value)
    if (next.has(id)) next.delete(id); else next.add(id)
    admin2Selected.value = next
    const m = map.value
    if (!m) return
    const sel = [...next].filter(s => s.startsWith(iso3))
    if (m.getLayer(`admin2-${iso3}-sel`)) {
      m.setFilter(`admin2-${iso3}-sel`, ['in', ['get', '_admin2_id'], ['literal', sel]])
    }
  }
}
const admin2ClickHandlers = { IND: null, CHN: null }

async function refreshAdmin2() {
  const m = map.value
  if (!m || !isMapReady.value) return
  if (!admin2Enabled.value) {
    for (const iso3 of ['IND', 'CHN']) {
      const id = `admin2-${iso3}`
      try {
        if (admin2ClickHandlers[iso3]) { m.off('click', `${id}-fill`, admin2ClickHandlers[iso3]); admin2ClickHandlers[iso3] = null }
        if (m.getLayer(`${id}-sel`))     m.removeLayer(`${id}-sel`)
        if (m.getLayer(`${id}-outline`)) m.removeLayer(`${id}-outline`)
        if (m.getLayer(`${id}-fill`))    m.removeLayer(`${id}-fill`)
        if (m.getSource(id))             m.removeSource(id)
      } catch (_) {}
    }
    admin2Loaded.value = { IND: false, CHN: false }
    return
  }
  admin2Loading.value = true
  admin2Error.value = ''
  try {
    for (const iso3 of ['IND', 'CHN']) {
      const fc = await loadAdmin2(iso3)
      const id = `admin2-${iso3}`
      if (!m.getSource(id)) {
        m.addSource(id, { type: 'geojson', data: fc, promoteId: '_admin2_id' })
        m.addLayer({
          id: `${id}-fill`, type: 'fill', source: id,
          paint: { 'fill-color': iso3 === 'IND' ? '#3498db' : '#9b59b6', 'fill-opacity': 0.18 },
        })
        m.addLayer({
          id: `${id}-outline`, type: 'line', source: id,
          paint: { 'line-color': iso3 === 'IND' ? '#3498db' : '#9b59b6', 'line-width': 0.4, 'line-opacity': 0.7 },
        })
        m.addLayer({
          id: `${id}-sel`, type: 'fill', source: id,
          paint: { 'fill-color': '#f59e0b', 'fill-opacity': 0.55 },
          filter: ['in', ['get', '_admin2_id'], ['literal', []]],
        })
        const handler = admin2ClickHandlerFactory(iso3)
        admin2ClickHandlers[iso3] = handler
        m.on('click', `${id}-fill`, handler)
      }
      admin2Loaded.value[iso3] = true
    }
  } catch (e) {
    admin2Error.value = String(e.message || e)
  } finally {
    admin2Loading.value = false
  }
}

watch(admin2Enabled, refreshAdmin2)
watch(isMapReady, (ready) => { if (ready && admin2Enabled.value) refreshAdmin2() })

function exportAdmin2DraftList() {
  const picks = [...admin2Selected.value]
  if (picks.length === 0) return
  const payload = {
    generated: new Date().toISOString(),
    note: 'Draft Asia list — admin-2 ids picked on the map. Run spatial intersection vs UPG lat/long in v2 to bind UPGs to units.',
    admin2_ids: picks,
    count_admin2_units: picks.length,
  }
  const blob = new Blob([JSON.stringify(payload, null, 2)], { type: 'application/json' })
  const url = URL.createObjectURL(blob)
  const a = document.createElement('a')
  a.href = url; a.download = `asia-draft-list-${Date.now()}.json`; a.click()
...
<template>
  <div ref="rmRoot" :class="['rm-root', { 'rm-dark': isDark, 'dsm-dark': isDark }]">
    <!-- CLONE EDIT (my-upg-100-list bundle): admin-2 toggle for India + China. -->
    <div class="admin2-control" :class="{ active: admin2Enabled }">
      <label>
        <input type="checkbox" v-model="admin2Enabled" />
        <span>Admin-2 (India + China)</span>
      </label>
      <div class="admin2-status" v-if="admin2Loading">Loading geoBoundaries… (~15MB)</div>
      <div class="admin2-status err" v-else-if="admin2Error">{{ admin2Error }}</div>
      <div class="admin2-status" v-else-if="admin2Enabled">
        IND: {{ admin2Loaded.IND ? '736 districts' : '…' }} ·
        CHN: {{ admin2Loaded.CHN ? '2391 counties' : '…' }}
        <span v-if="admin2Selected.size > 0"> · <b>{{ admin2Selected.size }} picked</b></span>
      </div>
      <div class="admin2-actions" v-if="admin2Enabled">
        <button :disabled="admin2Selected.size === 0" @click="exportAdmin2DraftList">Export draft Asia list</button>
        <button :disabled="admin2Selected.size === 0" @click="admin2Selected = new Set()">Clear picks</button>
      </div>
    </div>
    <!-- 10-tab bar -->
