# src/ — doxa-simple-map's sandbox

This is **this map's** folder. Everything the simple map needs that isn't
already in the shared `library/` gets built right here. Nothing in this folder
leaks to any other map — build freely. All subfolders are empty scaffolds today
(doxa-simple-map ships no local code and no local color strategies) — they mark
where things go when the map grows.
...
2. **Use your own code from this sandbox** → import with a **relative path**
   ...
   ```js
   // from doxa-simple-map.vue (sits at the bundle root, next to index.js):
