...
> **Status:** shipped (2026-06-24). Lives in the shared template (`src/`), so every
> profile that includes the two edge strips gets it automatically.
> **Code:** `src/composables/useMapInstance.js` (the `Edge scroll-affordance strips`
> block, ~line 250) + `.map-scroll-edge` strips in each profile's template/CSS +
> `user-select:none` on `src/components/map-controls/MapControlButton.vue`.
...
- **The text-selection bug:** a drag near the right edge used to **select the control
  buttons' glyphs (the emoji) to copy**, because the buttons overlap the strip. Fix =
  `user-select:none` on both the strips and the `.mcb` buttons (kept even though the
  desktop grab handler is gone, so a stray drag never selects anything).
