# REFERENCE-wiki — the 1040-maps app-profile bundle system
...
The whole repo is split into **three zones that are never mixed:**

| Zone | Path | Alias | License | You edit it… |
|---|---|---|---|---|
| **Shared template library** | `library/` | `@map` | Apache-2.0 | …once; the fix reaches every bundle on next build |
| **Your app-profile** | `app-profiles/<name>/` | — | yours | …freely; nothing here leaks to other maps |
| **The build machine** | `internal/` | — | Apache-2.0 | …never — it's the bundler's own scripts |

The shared library (`library/`, imported everywhere as `@map`) holds the reusable
map machinery: components (legends, toolbar, drawers), composables
(`useMapInstance`, `useMapData`, `useMapLayers`, …), colors (the color-strategy
registry), api (the data-source seam), config, constants, i18n (11 base locales),
Pinia stores, and utils. **Never bake one
map's private choices into `library/`** — it is common to everyone.
...
```
1040-maps/
├── library/                      ← SHARED TEMPLATE LIBRARY (alias @map, Apache-2.0)
...
│   │   └── legends/SemanticTreeLegend.vue  ← hierarchical one-to-many legend (see §5)
│   ├── composables/              ← useMapInstance, useMapData, useMapLayers, … (+ legends/, poster/)
│   ├── api/                      ← DataSourceManager (csv | api | rest-api) + sources.json
│   ├── colors/                   ← one color strategy per file, auto-registered by _registry.js
│   ├── constants/                ← never-change values: mapDefaults.js, zoom.js, posterDefaults.js, posterSizes.js
│   ├── i18n/locales/             ← 11 base locales: ar de en es fr hi it pt ro ru zh
│   ├── stores/                   ← Pinia: dataStore, mapStore, uiStore (per-instance)
│   └── utils/                    ← pure helpers (geoUtils, clustering)
│
...
│
├── internal/                     ← the bundler's own build scripts (generate-tree.mjs, clean.mjs,
│                                    checks) — never edited by map builders
...
> **Where the shared library lives:** the shared library ships as **`library/`**,
> aliased **`@map`** in `vite.config.js`. Import from `@map/...`.
...
```bash
BUNDLE=my-new-map bun run build:bundle   # builds just yours → public/js/my-new-map.js
# or: bun run build                       # loops & builds every bundle
```
...
- `dataSource` is handed to `@map/api/DataSourceManager`, which supports three
...
`@map/components/legends/SemanticTreeLegend.vue` is a **reusable hierarchical legend for
...
`useXxxLegendData` composable in `@map/composables/legends/`), pass it in, and react to
`@select`.
...
```jsonc
"dev":          "bun x vite",               // dev server, all bundles at their html paths
"build":        "bun run clean && for d in app-profiles/*/; do BUNDLE=$(basename \"$d\") bun x vite build || exit 1; done && bun run tree",
"build:bundle": "bun x vite build",         // builds ONE bundle (needs BUNDLE=<name>)
"build:watch":  "bun x vite build --watch",
"clean":        "rm -rf app/doxa-maps && mkdir -p app/doxa-maps",
"tree":         "node generate-tree.mjs"    // manifest + static staging HTML tree
```

> `bun x vite` runs the project-local Vite without a global install. `npm run …`
> works identically (npm resolves the same scripts; Bun just has to be on PATH).
...
**IIFE** (`inlineDynamicImports: true`). `bun run build` simply loops over every
...
> **The one rule that surprises everyone (from CONTRIBUTING.md):** `bun run dev`
...
> change in the parent app you must **`bun run build`** (or `build:bundle`), then
...
> After `bun run build`, refresh the relevant page there to see your change.
...
Maps read their data through `@map/api/` (the engine) driven by
`@map/api/sources.json` (the declaration). `sources.json` lists every
...
front door is `api/_registry.js`. Three connection types are supported:

| Type | What | Where it points |
|---|---|---|
| `rest-api` | versioned REST endpoint (the default) | **pray-tools** — `pray.doxa.life` live API for people-group prayer / engagement / adoption counts. Base URL resolved at runtime from `window.MAP_APP_API_URL` → `VITE_API_BASE_URL` → `''` (see `api/apiBaseUrl.js`), so the host sets it with no rebuild. |
...
**Add or change a source:** edit `library/api/sources.json` (declaration only) and,
if it needs new field names, extend its `fieldMappings` — components stay
source-agnostic. See `library/api/README.md` for the full contract. Prefer
...
| Fix a legend / control for ALL maps | edit it once in `library/` (`@map`); rebuild bundles |
| Add a color strategy | new file in `@map/colors/`, auto-registered by `_registry.js` (map-private ones go in your bundle's `src/colors/`) |
| Build everything | `bun run build` → `public/js/*.js` |
| Build just one | `BUNDLE=<name> bun run build:bundle` |
| Preview in isolation | `bun run dev` → `http://localhost:5173/app-profiles/<name>/index.html` |
| See a change in the parent app | `bun run build`, then refresh the Nuxt page (NOT `dev`) |
| Embed in a Nuxt page | `<DoxaMapSlot :profile-config="…" :bundle="…" />` (§7) |

## 10. See also

- `HOWTO-build-a-map.md` (this `docs/` folder) — the 3-zone quick-reference: where to make common edits + what NOT to do
...
- `library/ProfileLoader.vue` — the profile-config → screen loader (the runtime contract)
