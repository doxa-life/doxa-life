...
| | Path | Role |
|---|---|---|
| **The shared library** | `src/` (Vite alias `@map`) | One Apache-2.0 template of building blocks: map components, composables, color strategies, API connectors, stores, i18n. Reusable across every map. |
...
```
src/                          ← @map — the shared library (fix once, every map benefits)
...
`src/` provides the blocks; your profile composes them. The only place a map's identity
lives is its `app-profiles/<bundle>/` folder; never fork `src/` per map.
...
- `resolve.alias['@map'] = src/` — the `@map` import prefix points at the shared library.
...
`src/ProfileLoader.vue` (`@map/ProfileLoader.vue`) is the **only** component that reads
...
A map is yours through exactly three seams. Pick or extend a seam; never fork `src/` per
map.

### Seam 1 — Color strategies (`src/config/color-strategies/`)
...
### Seam 2 — API connections (`src/api-connections/` + `src/config/sources.json`)
...
### Seam 3 — i18n (`src/i18n/`)

Translated strings via `useI18n()`. Eleven base locales ship in `src/i18n/locales/`
...
## 7. Library-vs-profile rule + `src/` folder map

**The rule:** if a thing is reusable across **2+ profiles**, it belongs in `src/` (`@map`);
...
`src/` to customize a single map — use the three seams above.

`src/` (alias `@map`) groups:
...
| `config/` | Map + color config: `mapDefaults.js`, `colors.js`, `zoom.js`, **`color-strategies/`** (Seam 1), `sources.json` (Seam 2). |
| `api-connections/` | `DataSourceManager.js` + connectors + `_registry.js` (Seam 2). |
...
composes the `src/components/dashboards/` blocks (search header, facet rail, charts rail,
