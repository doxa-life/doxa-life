...
### Deploy your own — give the map a home on the internet

The script tag above loads the map from *this* site's `public/js/`. But the same map can live
on **your** address — your own little map-hosting corner that hundreds of teams can embed
without anyone ever rebuilding. That's what the **`starter-kit/`** folder is for:

- **`starter-kit/README.md`** — the team-facing one-pager: get the script tag, and fill in
  `profile-template.json` (which map, your Mapbox token, your data). Changing that JSON never
  needs a rebuild.
- **`starter-kit/deploy-guide.md`** — four ways to host the built bundle yourself, one page
  each: **Railway**, **Coolify**, **plain nginx**, or **S3 + CloudFront**. Pick the one that
  matches what you already use.

The beautiful part: you host the bundle **once**, and every team that embeds it just edits
their own little `profile-config` JSON. One map file, hosted in one place, serving a hundred
teams — each with their own token, data, and colors, and **not one of them ever has to build
anything.** That's the whole "no-rebuild map hosting" idea, made concrete.
...
And behind all of it sits the **shared library** — the project's `library/` folder, which your
