...
const LegendFamilyTree  = defineAsyncComponent(() => import('./LegendFamilyTree.vue'))
...
<template>
  ...
  <div v-else :class="['legend-container', { collapsed: isCollapsed, 'legend-dark': isDark }]">
    ...
    <div class="legend-content" :class="{ 'no-padding': legendColumns.length > 0 }">
      ...
      <!-- Language Families tab: expand/collapse tree view.
           Inject the card-level collapse caret into the #title-caret slot so
           this tab can collapse to the 36px pill like the other tabs. -->
      <LegendFamilyTree
