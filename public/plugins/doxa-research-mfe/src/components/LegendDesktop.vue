...
// TODO: port useLegendData composable — required for data-driven legend rows
...
/* When collapsed, hide everything EXCEPT the title inside LegendRows.
   The title row stays visible; column labels are hidden so the collapsed
   state shows only "Legend". */
...
const props = defineProps({
   ...
   * 'pray' | 'adopt' | 'none' — semantics defined by the consuming app.
   */
  popupAction: {
    type: String,
    default: 'pray',
...
// Instance ID — injected per map instance. Used to scope window events so
// multiple maps on the same page don't interfere with each other.
const mapId = inject('mapId', null)

// Platform detection
const isMobile = computed(() => uiStore.isMobile)

// Legend mode (data view ↔ detail view)
...
// - COLLAPSED → "Legend"
// - OPEN → the data-driven title (e.g., "Prayer Progress")
// - DETAIL mode → the selected people group's name
...
// TODO: auto-fly button visibility is currently tied to DOXA-specific
// legend type strings. Consumer apps should override this predicate or
...
onMounted(() => {
  ...
  // Only respond to events from our own map instance (matched by mapId).
  // TODO: 'doxa:openDesktopLegend' event name contains the DOXA brand — rename
  // to a generic namespace (e.g. 'map:openDesktopLegend') when consolidating.
...
<template>
  ...
  <!-- Desktop: single table, no separate header bar.
       The legend title is rendered by LegendRows as the first row of the same
       grid as the data rows. Collapse caret is absolutely positioned in the
       card's LEFT PADDING area (outside the grid). -->
    ...
    <div class="legend-content" :class="{ 'dropdown-active': showClusterSettings || showSpeedSelector, 'no-padding': legendColumns.length > 0 }">
      ...
      <div v-if="showAutoFlyButton" class="legend-controls">
        ...
        <div class="control-btn-group" :class="{ 'dropdown-open': showClusterSettings }">
          ...
          <!-- TODO: cluster-mode strings below are DOXA-specific. Generalize
               into a configurable set of cluster options passed via prop. -->
      ...
      <!-- Language-family tab: expand/collapse tree with carat semantics
           merged from the OLD legend onto the NEW CSS-table chrome. Owns its
           own row data via useLanguageFamilyLegendData; everything else
           (collapse caret, dark mode, header) stays in this card. -->
      <LegendFamilyTree
        v-else-if="legendType === 'language-family'"
        :peopleGroups="dataStore.normalizedPeopleGroups || legendDataItems"
        :title="legendTitle"
      ...
      <!-- Universal data-driven legend rows.
           ...
           off. When the tree/expand UI is ready, change this back to
...
<style scoped>
/* All runtime styles are injected via useShadowStyles above — this scoped
   block is kept for IDE tooling and historical reference only. */
</style>
