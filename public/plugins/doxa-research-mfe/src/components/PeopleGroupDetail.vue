<template>
  <div class="people-group-detail" :class="{ 'dark-mode': dark }">
    <!-- Header with Avatar, Name, and X Button (hidden when parent handles the header) -->
    ...
    <div class="detail-content">
      <!-- Name lead — shown when parent hides its own header. -->
      ...
      <!-- Quick Stats — Country, Population, Language, Religion. Em-dash fallback. -->
...
// TODO: port useAvatarGenerator composable — produces the per-PG avatar letter+color
...
const props = defineProps({
   ...
   *   'pray'  → opens pray-flow URL for the slug
   *   'adopt' → opens adopt-flow URL on current origin
   *   'none'  → primary button hidden; only "More info" shown
   * TODO: action URLs are currently hardcoded with DOXA-specific hosts/paths
   * (pray.doxa.life, /adopt/<slug>/, /research/<slug>/). Extract to props or
   * an injected URL-template config so consumer apps can override.
   */
  action: {
    type: String,
    default: 'pray',
...
// Fetches richer detail on each pin click; falls back to bulk data.
// ─────────────────────────────────────────────────────────────────
...
async function fetchDetail(slugVal) {
  ...
  try {
    // TODO: detail API path is DOXA-specific. Generalize to a configurable
    // endpoint template (e.g. injected `detailUrlTemplate` or per-source).
...
// Country — pick the most-decoded label available, with raw-CSV fallbacks.
...
const language = computed(() => {
  ...
  // Already a proper object with a label
...
// Religion — prefer the unified decoded label.
...
// TODO: NO_IMAGE_URL is hosted on a third-party domain (peoplegroups.org).
// Replace with a generic local placeholder asset for the framework.
...
function handleClose() {
  uiStore.selectPeopleGroup(null); // Clears selection and returns to data-view mode
}

// Slug-based URL templates.
// TODO: these URL builders are DOXA-specific (pray.doxa.life host, /adopt/
// and /research/ paths on current origin). Replace with a configurable
// URL-template service injected from the consumer app.
