...
// 
// QA Reference: Round 1 Q4 - Composable decomposition
// QA Reference: Round 3 Q5 - Performance (data duplication across maps)
// Design Flaw Fix: Extracts data loading from MapComponent, integrates with dataStore
//
// REFACTORED: Now uses NPM Vue package instead of vendored libs
// dataSourceManager must be passed in since it's loaded via ES module import
 ...
 * - initDataSource() (lines 309-371)
 * - loadRegionsData() (lines 504-513)
 * - loadLanguageFamiliesData() (lines 515-527)
 * 
...
export function useMapData(options) {
    ...
    async function loadData() {
        ...
        try {
            ...
            // This is for LEGEND display - shows total UPGs with each resource
            // Note: A UPG with both Bible and Jesus Film will be counted in BOTH categories
            ...
            // Each resource has a "No [Resource]" child to show gaps
            // "Has Resources" has "No Resources" as its child
     ...
     * @returns {Object} { familyName: [peopleGroups] }
     */
     ...
     * @returns {Object} { regionName: [peopleGroups] }
     */
    ...
    /**
     * Get people groups for a specific family
     * @param {string} familyName - Language family name
     * @returns {Array} People groups in the family
     */
    ...
    /**
     * Get people groups for a specific region
     * @param {string} regionName - DOXA region name
     * @returns {Array} People groups in the region
     */
    ...
    return {
        // Reactive State
        isLoading,              // ref<boolean> - Loading in progress
        isInitialized,          // ref<boolean> - Data has been loaded at least once
        error,                  // ref<string|null> - Last error message
        
        // Data Arrays (reactive refs, but content is marked as raw)
        peopleGroups,           // ref<Array> - Backward-compatible format
        normalizedPeopleGroups, // ref<Array> - DataSourceManager normalized format
        regionsData,            // ref<Object> - GeoJSON regions data
        languageFamiliesData,   // ref<Array> - Language families with counts
        doxaRegionsData,        // ref<Array> - DOXA regions with counts
        resourcesData,          // ref<Array> - Gospel resources data
        stats,                  // ref<Object> - Data loading statistics
        
        // Methods
        loadData,               // () => Promise - Load all data from source
        loadRegionsData,        // () => Promise - Load region GeoJSON separately
        getDataByLanguageFamily,// () => Object - Get groups by family
        getDataByRegion,        // () => Object - Get groups by region
        getPeopleGroupsByFamily,// (name) => Array - Get groups for family
        getPeopleGroupsByRegion,// (name) => Array - Get groups for region
        clearData               // () => void - Clear all data
    };
}

// ES Module export (for import statements in other modules)
// Note: Vue/Pinia are global, but our own code uses ES modules
